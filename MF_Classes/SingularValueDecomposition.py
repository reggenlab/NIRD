...
class SingularValueDecomposition(MatrixFactorization):
	...
	def _get_factorizer(self):
		return TruncatedSVD(n_components=self.n_components, random_state=123)
	...
	def factorize(self):
		_svd = self.factorizer
		_svd.fit(self.scaled_exp)

		self.low_rank_exp = _svd.transform(self.scaled_exp)
		self.mixture_matrix = abs(_svd.components_)
	...
	def low_rank_model(self):
		...
		# # Single Loop
		# self.fis = [self.data2network(ip) for ip in clubbed_ip]
