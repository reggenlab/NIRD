...
class Eval_EdgeOverlapping(MF_Evaluator):
    def __init__(self, network1, network2, method=None, top=10000, dx=500, outdir="./") -> None:
    ...
    def evaluate(self):
        plot_path = os.path.join(self.outdir, f"{self.method}_edge_overlapping.svg")
    ...
    def get_matching_edges(self, plot=True, f_name='plot.png'):
        ...
        for xt in range(0, g2.shape[0], self.dx):
            ...
            df_temp = df_temp.append({'xt': xt, 'matches': (len(set(zip(*e1)) & set(zip(*e2))))},
                                     ignore_index=True)
    ...
    def plot_matching_edges(self, df_match, f_name='plot.png'):
        ...
        p.set(
            xlabel="Edges in smartSeq",
            ylabel="Matches in dropSeq (in %)",
            title=method_name + "Edge overlapping " +
                  self.network1['_label'] + " and " + self.network2['_label']
        )
        plt.savefig(f_name)
        plt.close()
